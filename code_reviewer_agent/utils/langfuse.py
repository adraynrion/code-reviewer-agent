...
class LangfuseKey(StringValidator): pass

class LangfuseHost(StringValidator): pass
...
class LangfuseModel:
    ...
    def tracer(self) -> Tracer:
        if not self._tracer:
            raise ValueError("Langfuse is not initialized. Please correctly initialize it first.")
        return self._tracer
    ...
    def enabled(self, value: bool):
        ...
        try:
            if not self.public_key or not self.secret_key or not self.host:
                raise ValueError("Langfuse is not entirely configured. Missing public_key, secret_key or host.")
            ...
            logfire.configure(
                ...
                scrubbing=logfire.ScrubbingOptions(callback=self.__class__.scrubbing_callback),
            )
