...
class BaseUrl(StringValidator): pass

class ModelProvider(StringValidator): pass

class LLM(StringValidator): pass
...
class LLMDict:
    ...
    @config.setter
    def config(self, config: LLMConfig):
...
class AiModel:
    ...
    def provider(self, value: ModelProvider):
        ...
        if value not in ("OpenAI", "TogetherAI", "OpenRouter", "Google", "Ollama"):
            raise ValueError("Provider must be one of: OpenAI, TogetherAI, OpenRouter, Google, Ollama")
...
class AiModelType(type):
    """ Type for AiModel classes. Ensures that all classes inherit from AiModel. """
