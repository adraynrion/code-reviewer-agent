...
from code_reviewer_agent.services.repository import CodeDiff, Files, Repository, RepositoryService, RequestId
...
class GitLabToken(StringValidator): pass

class GitLabApiUrl(StringValidator): pass


class GitLabReviewerService(RepositoryService):
    def __init__(self, repository: Repository, request_id: RequestId, config: ReviewerConfig):
    ...
    def request_files_analysis_from_api(self) -> None:
        ...
        try:
            ...
            mr_metadata_url = (
                f"{self.gitlab_api_url}/projects/{self.repository}/merge_requests/{self.request_id}"
            )
    ...
    async def post_review_comments(self, diff: CodeDiff, reviewer_output: CodeReviewResponse) -> None:
        print_section(f"Posting to GitLab repo {self.repository} MR #{self.request_id}", "📝")
        headers = {
    ...
    def _assign_reviewed_label(self):
        ...
        print_section(f"Updating MR #{self.request_id} of GitLab repo {self.repository}", "🏷️")
        headers = {
            "Private-Token": self.gitlab_token,
            "Content-Type": "application/json",
        }
        ...
        if resp.status_code != 200:
            raise Exception(f"Failed to get authenticated user: {resp.status_code} - {resp.text}")

        user_data = resp.json()
        user_id = user_data.get("id")
        user_name = user_data.get("name", f"User {user_id}")
        if not user_id:  # Only check user_id as it is the only field required for the update request
            raise Exception("Could not determine GITLAB_TOKEN owner. 'id' field missing in response.")
        ...
        if resp.status_code != 200:
            raise Exception(f"Failed to set reviewer on MR: {resp.status_code} - {resp.text}")
