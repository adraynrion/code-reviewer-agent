...
from code_reviewer_agent.models.base_types import StringValidator, PositiveIntegerValidator
...
class Platform(StringValidator): pass

class Repository(StringValidator): pass

class RequestId(PositiveIntegerValidator): pass

class InstructionsPath(StringValidator): pass

class Token(StringValidator): pass

class Url(StringValidator): pass
...
class CodeReviewService(BaseService):
    ...
    def __init__(self, platform: Platform, repository: Repository, request_id: RequestId, instructions_path: InstructionsPath):
        ...
        self._instructions_path = instructions_path or reviewer_config.instruction_dir_path
    ...
    def instructions_path(self, value: str):
    ...
    def repository_service(self, platform: Platform):
        if platform == "github":
            self._repository_service = GitHubReviewerService(self.repository, self.request_id, self._config.schema.reviewer)
        elif platform == "gitlab":
            self._repository_service = GitLabReviewerService(self.repository, self.request_id, self._config.schema.reviewer)
        else:
            raise ValueError("Invalid platform. Must be either 'github' or 'gitlab'.")

    async def _file_code_review_without_langfuse(self, diff: CodeDiff, user_input: str) -> None:
    ...
    async def _file_code_review(self, diff: CodeDiff, filename: str, languages: List[str], user_input: str) -> None:
        with self.langfuse.tracer.start_as_current_span("CR-Agent-Review") as file_review_span:
            file_review_span.set_attribute("langfuse.user.id", f"cr-request-{self.request_id}")
            file_review_span.set_attribute("langfuse.session.id", f"cr-{self.platform}-{self.repository}")
            file_review_span.set_attribute("filename", filename)
            file_review_span.set_attribute("languages", languages)

            reviewer_output = await self._file_code_review_without_langfuse(diff, user_input)

            file_review_span.set_attribute("input.value", user_input)
            file_review_span.set_attribute("output.value", reviewer_output)

    async def _review_files_without_langfuse(self) -> None:
        files_reviewed = 0
    ...
    async def _review_files(self) -> None:
        with self.langfuse.tracer.start_as_current_span("CR-Agent-Main-Trace") as main_span:
            ...
            main_span.set_attribute("input.value", self.repository_service.diffs)
            main_span.set_attribute("output.value", f"Reviewed {files_reviewed}/{len(self.repository_service.diffs)} files successfully")
    ...
    async def main(self) -> None:
        ...
        try:
            ...
            print_error(f"Error fetching {'PR' if self.platform == 'github' else 'MR'} files diffs: {str(e)}")
            print_exception()
            sys.exit(1)

        # ========== Reviewer Agent: loop on each file diff ==========
        print_section("Starting Code Review process", "🤖")
        print_debug(f"Reviewing {len(self.repository_service.diffs)} file(s) with changes")
