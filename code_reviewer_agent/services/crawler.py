...
from code_reviewer_agent.models.base_types import PositiveFloatValidator, PositiveIntegerValidator
...
class ApiKey(StringValidator): pass

class EmbeddingModel(StringValidator): pass
...
class MaxPages(PositiveIntegerValidator): pass

class MaxDepth(PositiveIntegerValidator): pass

class ConcurrentTasks(PositiveIntegerValidator): pass

class ExtractionType(StringValidator): pass

class ChunkTokenThreshold(PositiveIntegerValidator): pass

class OverlapRate(PositiveFloatValidator): pass

class Temperature(PositiveFloatValidator): pass

class MaxTokens(PositiveIntegerValidator): pass

class Headless(bool): pass
...
class Locale(StringValidator):
    def __set__(self, instance, value: str):
        ...
        try:
            ...
            raise ValueError(f"Invalid locale. Must be a valid BCP 47 language tag. Got {formated_value}")
...
class Timezone(StringValidator):
    def __set__(self, instance, value: str):
        ...
        if gettz(formated_value) is None:
            raise ValueError(f"Invalid timezone. Must be a valid timezone string. Got {formated_value}")

        super().__set__(instance, formated_value)

class Keywords(List[str]): pass

class KeywordWeight(PositiveFloatValidator): pass
...
class CrawlService(BaseService):
    ...
    def __init__(self, **kwargs):
        ...
        self._concurrent_tasks = kwargs.get("concurrent_tasks") or crawler_config.concurrent_tasks
        self._extraction_type = kwargs.get("extraction_type") or crawler_config.extraction_type
        self._chunk_token_threshold = kwargs.get("chunk_token_threshold") or crawler_config.chunk_token_threshold
        ...
        self._keywords = kwargs.get("keywords") or crawler_config.keywords or (
            "crawl",
            "example",
            "best practices",
            "configuration",
        ...
        self._keyword_weight = kwargs.get("keyword_weight") or crawler_config.keyword_weight
    ...
    def extraction_type(self, value: str):
        ...
        if formated_value not in ("schema", "block"):
            raise ValueError("Invalid extraction type. Must be either 'schema' or 'block'.")
    ...
    def chunk_token_threshold(self, value: int):
    ...
    @overlap_rate.setter
    def overlap_rate(self, value: float):
    ...
    async def main(self) -> None:
        ...
        self.agents.writter.crawl_urls(
            urls=self.urls,
