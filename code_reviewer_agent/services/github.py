...
class GitHubToken(StringValidator): pass
...
class GitHubReviewerService(RepositoryService):
    ...
    def request_files_analysis_from_api(self) -> None:
        ...
        try:
            ...
            pr_metadata_url = (
                f"https://api.github.com/repos/{self.repository}/pulls/{self.request_id}/commits"
            )
    ...
    async def post_review_comments(self, diff: CodeDiff, reviewer_output: CodeReviewResponse) -> None:
        print_section(f"Posting to GitHub repo {self.repository} PR #{self.request_id}", "📝")
        headers = {
    ...
    def _assign_reviewed_label(self):
        ...
        print_section(f"Updating PR #{self.request_id} of GitHub repo {self.repository}", "🏷️")
        headers = {
            "Authorization": f"token {self.github_token}",
            "Accept": "application/vnd.github.v3+json",
        }
        ...
        if "login" not in user_data:
            raise Exception("Could not determine GITHUB_TOKEN owner. 'login' field missing in response.")

        token_owner = user_data["login"]
        print_info(f"Setting reviewer as {token_owner} on PR...")
        review_url = (
            f"https://api.github.com/repos/{self.repository}/pulls/{self.request_id}/requested_reviewers"
        )
        review_data = {"reviewers": [token_owner]}
        resp = requests.post(review_url, headers=headers, json=review_data)
        if resp.status_code != 201:
            raise Exception(f"Failed to set reviewer on PR: {resp.status_code} - {resp.text}")
