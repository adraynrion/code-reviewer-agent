...
from code_reviewer_agent.utils.rich_utils import print_debug, print_info, print_section, print_success, print_warning

class Repository(StringValidator): pass

class RequestId(PositiveIntegerValidator): pass

class CommitSha(StringValidator): pass

class Files(Tuple[Dict[str, Any]]): pass

class FilesDiff(List[Dict[str, Any]]): pass

class CodeDiff(Dict[str, Any]): pass

class Languages(Dict[str, Set[str]]): pass

class Label(StringValidator): pass
...
class RepositoryService:
    ...
    def __init__(self, repository: Repository, request_id: RequestId):
    ...
    def diffs(self, files: Files):
        ...
        for file in files:
            ...
            if not self.languages.get(filename):
                print_warning(f"Skipping file analysis {filename}: no languages detected")
                continue
    ...
    @languages.setter
    def languages(self, value: Languages):
    ...
    def last_commit_sha(self, sha: str):
    ...
    @abstractmethod
    async def post_review_comments(self, diff: Dict[str, Any], reviewer_output: CodeReviewResponse) -> None:
        raise NotImplementedError
