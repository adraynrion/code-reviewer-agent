...
class CrawlerWritter:
    ...
    def extraction_strategy(self, extraction_strategy: LLMExtractionStrategy):
        if not isinstance(extraction_strategy, LLMExtractionStrategy):
            raise ValueError("Extraction strategy must be an instance of LLMExtractionStrategy")
    ...
    async def crawl_urls(self) -> CrawledDocuments:
        ...
        print_debug(f"Crawling {len(self.urls)} URLs with max {self.max_pages} pages each")
        print_debug(f"Using keywords for relevance: {', '.join(self.keywords) if self.keywords else 'None'}")
        ...
        with Progress(
            ...
            main_task = progress.add_task("[cyan]Initializing crawler...", total=len(self.urls))
            ...
            try:
                    ...
                    try:
                        ...
                        for doc in docs:
                            ...
                            try:
                                ...
                                success = await self.store_doc(doc_data, progress, doc_task_id)
                                if success:
                                    results.append(doc_data)
                                    successful_crawls += 1
                                    description = f"[green]✓ Correctly stored: {document_title}"
                                else:
                                    description = f"[yellow]! Failed to store: {document_title}"
                ...
                progress.update(
                    main_task,
                    description=f"[red]! Error crawling {url}: {str(e)}"
                )
            finally:
                if successful_crawls == len(self.urls):
                    ...
                    description = "[yellow]! "
                description += f"Successfully crawled {successful_crawls}/{len(self.urls)} URLs"
                progress.update(
                    main_task,
